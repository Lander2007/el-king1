...
router.post('/orders', async (req, res) => {
    ...
    const validation = order_1.serverOrderSchema.safeParse(req.body);
    ...
    try {
        ...
        const settingsObj = await Settings_1.Settings.findOne();
...
router.get('/orders/:id', async (req, res) => {
    try {
        const order = await Order_1.Order.findById(req.params.id).populate('items.product');
        if (!order) {
...
router.get('/admin/orders', auth_1.adminMiddleware, async (req, res) => {
    try {
        const orders = await Order_1.Order.find().populate('items.product').sort({ createdAt: -1 });
        res.json(orders);
    }
    catch (error) {
        res.status(500).json({ message: error.message });
...
router.put('/admin/orders/:id', auth_1.adminMiddleware, async (req, res) => {
